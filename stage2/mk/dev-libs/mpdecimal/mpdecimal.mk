...
#	cd tmp/mpdecimal/bld && ../mpdecimal-$(MPDECIMAL_VER)/configure $(MPDECIMAL_OPT) && make $(JOBS) V=$(VERB) && make DESTDIR=`pwd`/../ins install
	cd tmp/mpdecimal/mpdecimal-$(MPDECIMAL_VER) && ./configure $(MPDECIMAL_OPT) && make $(JOBS) V=$(VERB) && make DESTDIR=`pwd`/../ins install
	rm -fr tmp/mpdecimal/ins/usr/share
ifeq ($(BUILD_STRIP),y)
	strip $(STRIP_BUILD_AST) tmp/mpdecimal/ins/usr/lib/*.a
	strip $(STRIP_BUILD_LIB) tmp/mpdecimal/ins/usr/lib/*.so*
endif
#	rm -fr tmp/mpdecimal/ins/usr/lib/*.a
