...
pkg/libatomic_ops.cpio.zst: src/libatomic_ops-$(LIBATOMIC_OPS_VER).tar.gz pkg/gzip.cpio.zst
	...
	rm -f tmp/libatomic_ops/ins/usr/lib/*.la
ifeq ($(BUILD_STRIP),y)
	strip $(STRIP_BUILD_LIB) tmp/libatomic_ops/ins/usr/lib/*.so*
endif
	mkdir -p pkg && cd tmp/libatomic_ops/ins && find . -print0 | cpio -o0H newc --quiet | zstd -z9T9 > ../../../$@
	rm -fr tmp/libatomic_ops
src/libatomic_ops-$(LIBATOMIC_OPS_VER).tar.gz: src/.gitignore
	wget -P src --no-check-certificate https://github.com/ivmai/libatomic_ops/releases/download/v7.8.2/libatomic_ops-$(LIBATOMIC_OPS_VER).tar.gz && touch $@
