...
pkg/binutils.cpio.zst: src/binutils-$(BINUTILS_VER).tar.xz pkg/expect.cpio.zst pkg/texinfo.cpio.zst pkg/elfutils.cpio.zst
	...
	cat pkg/elfutils.cpio.zst | zstd -d | cpio -idumH newc --quiet -D / > /dev/null 2>&1
	rm -fr tmp/binutils
...
#OK
	...
	cd tmp/binutils/bld && ../binutils-$(BINUTILS_VER)/configure $(BINUTILS_OPT) && make tooldir=/usr $(JOBS) V=$(VERB) && make tooldir=/usr DESTDIR=`pwd`/../ins install
